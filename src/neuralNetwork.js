...
class NeuralNetwork {
	...
	export() {
		return {
			inputLayerSize: this.layers[0].size,
			layers: Object.assign([], this.layers).splice(1, this.layers.length - 1).map((_layer) => {
				return {
					biases: _layer.biases.export(),
					weights: _layer.weights.export()
				}
			}),
			meta: {
	...
	import(_package) {
		this.layers = [new InputLayer(_package.inputLayerSize)];
		let prevLayer = this.layers[0];

		for (let layerData of _package.layers)
		{
			let layer = new Layer(layerData.biases.length, prevLayer);
			prevLayer = layer;
			layer.biases = new Matrix().fromArray(layerData.biases);
			layer.weights = new Matrix().fromArray(layerData.weights);
			this.layers.push(layer);
		}

		this.trainingUpdates = _package.meta.trainingUpdates;
		return this;
	}
